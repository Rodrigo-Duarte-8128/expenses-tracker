...
from datas.date import Date, date_is_in_interval, compare_dates
...
from screens.transactions_screens.add_income_options_screen import AddIncomeOptionsScreen
...
class AddIncomeScreen(MDScreen):
    def __init__(self, app, **kwargs):
        super(AddIncomeScreen, self).__init__(**kwargs)
        ...
        self.incomes_categories_dict = dict(filter(
            lambda pair: pair[1].kind=="Income",
            app.categories_screen.categories_dict.items()
            )
        )
        
        incomes_categories_names_list = list(self.incomes_categories_dict.keys())
        incomes_categories_names_list.sort()

        for category_name in incomes_categories_names_list:
            category = self.incomes_categories_dict[category_name]
        ...
        self.add_income_btn = MDRaisedButton(
            ...
            on_press = self.add_income(app)
        )
        self.layout.add_widget(self.add_income_btn)
    ...
    def options_clicked(self, app):
        def click(instance):
            app.add_income_options_screen = AddIncomeOptionsScreen(app, name="add_income_options_screen")
            app.screen_manager.add_widget(app.add_income_options_screen)

            app.transition_diagram.add_node(
                "add_income_options_screen",
                root_screen_node = app.home_screen_node,
                left_node_name = "add_income_screen"
            )

            app.switch_screen("add_income_options_screen")(instance)
        return click
    ...
    def add_income(self, app):
        def add(instance):
            ...
            if not errors:
                ...
                if int(new_transaction.date.year) == self.app.home_screen.displayed_month[1] and int(new_transaction.date.month) == self.app.home_screen.displayed_month[0]:
                    if self.app.home_screen.displayed_account == new_transaction.account:
                        ...
                        if len(self.app.home_screen.scroll_layout.children) != 0:
                            ...
                            while compare_dates(date, new_transaction.date.get_date_simple_string()):
                                index += 1
                ...
                app.transition_diagram.remove_node("add_income_screen")

                app.screen_manager.remove_widget(app.add_income_screen)
