...
class EditTransactionOptionsScreen(MDScreen):
    def __init__(self, app, transaction, target_screen_name, **kwargs):
        super(EditTransactionOptionsScreen, self).__init__(**kwargs)
        ...
        self.date_text_field.text = f"{self.transaction.date.day}/{self.transaction.date.month}/{self.transaction.date.year}"
        ...
        self.time_text_field.text = f"{self.transaction.date.hour}:{self.transaction.date.minutes}:{self.transaction.date.seconds}"
        ...
        self.note_text_field.text = transaction.note
        ...
        self.scroll_layout = MDGridLayout(
            cols=1,
            size_hint_y = None,
            size = (self.window_width, self.window_height * 0.425),
        )
        ...
        self.remove_btn = MDRaisedButton(
            ...
            on_press = self.remove_transaction(app)
        )
    ...
    def remove_transaction(self, app):
        def remove(instance):

            app.records_screen.remove_transaction(self.transaction.date.get_date_simple_string())

            app.switch_screen(self.target_screen_name)(instance)
            app.transition_diagram.remove_node("edit_transaction_options_screen")
            app.screen_manager.remove_widget(app.edit_transaction_options_screen)
    ...
    def account_pressed(self, account):
        def press(instance):
            ...
            if self.app.edit_transaction_screen.error_account_label in self.app.edit_transaction_screen.layout.children:
                self.app.edit_transaction_screen.layout.remove_widget(self.app.edit_transaction_screen.error_account_label)
    ...
    def confirm_options(self, app):
        def confirm(instance):
            ...
            if not errors:
                new_date = self.transaction.date.parse_string(f"{date} {time}")
                app.edit_transaction_screen.date = new_date
                app.edit_transaction_screen.note = note

                app.edit_transaction_screen.chosen_account_label.text = self.chosen_account_label.text
                if app.edit_transaction_screen.chosen_account_label not in app.edit_transaction_screen.layout.children:
                    app.edit_transaction_screen.layout.add_widget(app.edit_transaction_screen.chosen_account_label)

                app.switch_screen("edit_transaction_screen")(instance)
                app.transition_diagram.remove_node("edit_transaction_options_screen")
                app.screen_manager.remove_widget(app.edit_transaction_options_screen)
